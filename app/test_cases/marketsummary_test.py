...
def test_market_summary(market_url, username, password):
    # urls
    summaries_url = market_url + "/marketsummary"
    ...
    market_resp = requests.get(summaries_url, params=payload,
...
@pytest.mark.parametrize("username, password", [("admin_001", "admin_001@12345"), ("manager_001", "manager_001@12345")])
def test_market_summary_without_params(market_url, username, password):
    # urls
    summaries_url = market_url + "/marketsummary"
    login_url = market_url + "/login"
...
def test_market_summary_without_authtoken(market_url):
    # urls
    summaries_url = market_url + "/marketsummary"
...
def test_market_summary_invalid_token(market_url, token):
    # urls
    summaries_url = market_url + "/marketsummary"

    # market summary testcase
    payload = {"market": "btc-ltc"}
    market_resp = requests.get(summaries_url, params=payload, headers={'Authorization': token})
    out_result = market_resp.json()
